...
# unique list
vocab = np.unique(f_list) # no really need np, can use list(set(f_list)), see generator.py
...
# stem_u_vocab, count = np.unique(flat_list, return_counts=True)

# vocab = uniques
# vocab = {v: nlp.vocab[v].lower_ for v in uniques if v != skip}
# stem_u_vocab = {v: stem_u_vocab[v] for v in stem_u_vocab}
# vocab[skip] = '<SKIP>'
# tokens = data 
# print tokens, vocab
...
# print len(katadasar), len(stem_tokens), len(flat_list), len(stem_u_vocab), stem_u_vocab, count
# print len(tokens), len(vocab), tokens, vocab
del texts

# new_list = []
# new_list = [vocab[x] for x in count if x > 1]
# print new_list
