...
class LDA2Vec(Chain):
    ...
    def fit_partial(self, rsty_ids, rwrd_ids, window=5):
        # sty_ids, aut_ids, wrd_ids = move(self.xp, rsty_ids, raut_ids, rwrd_ids)
        sty_ids, wrd_ids = move(self.xp, rsty_ids, rwrd_ids)
        ...
        for frame in range(-window, window + 1):
            ...
            # sty_and_aut_are_same = np.logical_and(sty_is_same)
            # weight = np.logical_and(sty_and_aut_are_same, mask).astype('int32')
            weight = np.logical_and(sty_is_same, mask).astype('int32')
